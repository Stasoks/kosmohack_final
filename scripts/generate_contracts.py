...
EVENTS_PATH = ROOT / "shared_contracts/generated/events.py"
...
def registry_text(schema: dict[str, Any]) -> str:
    keys = sorted(
        (kind, version)
...
def main() -> None:
    ...
    _check_or_write(EVENTS_PATH, events_text(schema), check=args.check)
    _check_or_write(REGISTRY_PATH, registry_text(schema), check=args.check)
