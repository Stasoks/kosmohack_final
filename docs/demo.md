...
This sequence is designed for **Приёмочные сценарии** and uses only named acceptance bundles. After each run, distinguish **Рассчитанное состояние системы** from the acceptance checklist, then open the human item timeline; raw events and JSON remain available only in technical expanders. The independent **Симуляция производства** page is used for a live route-driven flow and is not a replacement for these fixed acceptance checks.
...
3. **S08 — controller to release (about 70 seconds).** In the activity timeline show controller confirmation, linked rework start/finish, repeat trusted GOOD, verification, and release. Then open **Контроль качества → Все несоответствия** and show the completed NCR read-only. The original occurrence remains in history.
