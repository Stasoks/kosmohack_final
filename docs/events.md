...
The source of truth is [the JSON Schema](../contracts/events/canonical-event.schema.json). Generated Pydantic payload models and the contract registry live in `shared_contracts/generated/`. `scripts/generate_contracts.py` renders both artifacts deterministically from the JSON Schema; `--check` regenerates into a temporary location and fails on drift. They are checked with:
